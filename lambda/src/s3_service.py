...
def get_s3_client():
    """Lazily initialize and cache the S3 client."""
    global _s3_client
...
def generate_processed_keys(original_key: str, width: Optional[int] = None) -> str:
    ...
    Raises:
        ValueError: If the key does not follow the originals/{userId}/{photoId} format
    ...
    if len(parts) < 3 or parts[0] != 'originals':
        raise ValueError(
            "Expected S3 key format 'originals/{userId}/{photoId}[.ext]', "
            f"received '{original_key}'"
        )
